...
CREATE TABLE TodoList(id serial not null primary key, owner_id int not null, version int not null default 0, CONSTRAINT unique_owner UNIQUE(owner_id));
ALTER TABLE TodoList ADD CONSTRAINT TodoListOwner_fk FOREIGN KEY (owner_id) REFERENCES Users(id);

CREATE TABLE TodoEntry(id serial not null primary key, priority int not null, list_id int not null, value varchar(2048) not null);
